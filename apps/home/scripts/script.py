        ...
        if "records" in response.keys():
            for app in response['records']:
                ...
                if OutBurst4 >= 80:
                    ...
                    o1 = OutInterfaceBurst(timestamp=datetime.fromtimestamp(timestamp), date=date_en, time=time_en, interface=interface, OutBurst1=OutBurst1, OutBurst2=OutBurst2, OutBurst3=OutBurst3, OutBurst4=OutBurst4, Burst3=0, Burst4=1)
                    #o1.applications = str(apps_dict)
                    o1.save()
                if InBurst4 >= 80:
                    in_computed=True
                    print(interface.deviceName, interface.name, InBurst1, InBurst2, InBurst3, InBurst4)
                    i1 = InInterfaceBurst(timestamp=datetime.fromtimestamp(timestamp), date=date_en, time=time_en, interface=interface, InBurst1=InBurst1, InBurst2=InBurst2, InBurst3=InBurst3, InBurst4=InBurst4, Burst3=0, Burst4=1)
                    #i1.applications = str(apps_dict)
                    i1.save()

                if in_computed or out_computed:
                    out_computed=False
                    in_computed=False
                    apps_throughput = throughput_per_interface_hourly(global_headers, interface.truview_if_id,
                                                                      interface.deviceIp, interface.deviceName,
                    ...
                    for record in apps_throughput['records']:
                        ...
                        if record['Application'] is None:
                            if 'UNKNOWN' in apps_dict.keys():
                                if record['TxThroughput'] is not None: apps_dict['UNKNOWN']['TxThroughput'] = \
                                    apps_dict['UNKNOWN']['TxThroughput'] + record['TxThroughput']
                                if record['RxThroughput'] is not None: apps_dict['UNKNOWN']['RxThroughput'] = \
                                    apps_dict['UNKNOWN']['RxThroughput'] + RxThroughput
                                ...
                                apps_dict['UNKNOWN'] = {'TxThroughput': TxThroughput, 'RxThroughput': RxThroughput,
                                                        'TotalThroughput': TotalThroughput}
                        else:
                            if record['Application']['name'] in apps_dict.keys():
                                if record['TxThroughput'] is not None: apps_dict[record['Application']['name']][
                                    'TxThroughput'] = apps_dict[record['Application']['name']][
                                                          'TxThroughput'] + TxThroughput
                                if record['RxThroughput'] is not None: apps_dict[record['Application']['name']][
                                    'RxThroughput'] = apps_dict[record['Application']['name']][
                                                          'RxThroughput'] + RxThroughput
                                if record['TotalThroughput'] is not None: apps_dict[record['Application']['name']][
                                    'TotalThroughput'] = apps_dict[record['Application']['name']][
                                                             'TotalThroughput'] + TotalThroughput
                            else:
                    ...
                    for key in apps_dict.keys():
                        AppInterfaceThrouput.objects.create(timestamp=datetime.fromtimestamp(timestamp), date=date_en,
                                                        time=time_en, interface=interface, inburst=i1, outburst=o1,
                                                        if_name=interface.truview_if_id,
                                                        router_name=interface.deviceName, app=key,
                                                        tx_throuput=apps_dict[key]['TxThroughput'],
                                                        rx_throuput=apps_dict[key]['RxThroughput'],
                                                        total_throuput=apps_dict[key]['TotalThroughput'])
                    """
def extract_monthly_data():
    authenticate_api()
    local_timestamp = 1672527600
    for hour in range(1, 2160):
        try:
